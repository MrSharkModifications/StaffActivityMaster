...
RegisterCommand('dutystaff', function(source, args, rawCommand)
	...
	if hasPerms[source] ~= nil then 
		if onDuty[source] == nil then 
			...
			TriggerClientEvent('StaffDutyScript:GiveWeapons', source);
		else 
			onDuty[source] = nil;
			local tag = activeBlip[source];
			local webHook = roleList[activeBlip[source]][3];
...
RegisterCommand('staff', function(source, args, rawCommand) 
	...
	sendMsg(source, 'The active Staff on are:')
...
RegisterCommand('bliptag', function(source, args, rawCommand)
	...
	if hasPerms[source] ~= nil then 
		...
		sendMsg(source, '^1ERROR: You must be an Staff Member on our discord to use this...')
	end 
end)


RegisterNetEvent('StaffDutyScript:RegisterUser')
AddEventHandler('StaffDutyScript:RegisterUser', function()
	local src = source
	...
	if identifierDiscord then
		...
		if not (roleIDs == false) then
			...
			print("[StaffDutyScript] " .. GetPlayerName(src) .. " has not gotten their permissions cause roleIDs == false")
		end
	else
		print("[StaffDutyScript] " .. GetPlayerName(src) .. " has not gotten their permissions cause discord was not detected...")
	end
	permTracker[src] = perms; 
end)